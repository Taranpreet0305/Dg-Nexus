...
const LoadingAnimation = memo(() => {
  ...
  useEffect(() => {
    if (document.readyState === 'complete') { setAssetsLoaded(true); return; }
    const handleLoad = () => setAssetsLoaded(true);
    window.addEventListener('load', handleLoad);
    const timeout = setTimeout(() => setAssetsLoaded(true), 2000);
    return () => { window.removeEventListener('load', handleLoad); clearTimeout(timeout); };
  }, []);
  ...
  useEffect(() => {
    const minTimer = setTimeout(() => { if (progress >= 100) setIsLoading(false); }, 1800);
      ...
      {isLoading && (
        ...
        >
          ...
          <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }}
            exit={{ opacity: 0, scale: 1.1 }} transition={{ duration: 0.4, ease: 'easeOut' }} className="relative z-10">
            <DGLogo size={80} className="text-foreground" />
          </motion.div>
          <motion.div className="relative mt-8 sm:mt-12 w-48 sm:w-64"
            ...
            <motion.div className="text-center mb-2 sm:mb-3">
              <span className="text-2xl sm:text-4xl font-display font-bold tracking-tight">{Math.round(progress)}</span>
            ...
            <div className="h-0.5 sm:h-1 bg-muted/30 rounded-full overflow-hidden">
              <motion.div className="h-full rounded-full bg-foreground" style={{ width: `${progress}%` }}
                transition={{ duration: 0.1 }} />
            </div>
            <motion.p className="text-center mt-3 sm:mt-4 text-[10px] sm:text-xs uppercase tracking-[0.2em] sm:tracking-[0.3em] text-muted-foreground"
