...
const workExperience = [
  { title: 'Graphic Designer', company: 'Merox', type: 'Work' },
  { title: 'Graphic Designer', company: 'Social Media Club', type: 'Work' },
];

const virtualInternships = [
  { title: 'Software Engineering', company: 'Accenture' },
  { title: 'Strategic & Experience Design', company: 'BCG X' },
  { title: 'Software Engineering', company: 'EA' },
  { title: 'UX Design', company: 'Lloyds Banking Group' },
];
    ...
    <section id="experience" className="py-16 sm:py-24 md:py-32 relative" ref={ref}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12">
        <motion.div className="text-center mb-10 sm:mb-16 md:mb-20"
          ...
          <span className="text-xs sm:text-sm uppercase tracking-[0.2em] sm:tracking-[0.3em] text-muted-foreground font-display">Journey</span>
            ...
            <div className="relative">
              ...
              <StaggeredContainer className="space-y-6 sm:space-y-8 md:space-y-10" staggerDelay={0.1} direction="left">
                {workExperience.map((exp) => (
                  <div key={exp.title + exp.company} className="relative pl-8 sm:pl-10">
                    <div className="absolute left-0 top-1.5 timeline-dot" />
                    <motion.div className="glass-card card-glow rounded-lg p-4 sm:p-5 md:p-6 hover:bg-card/80 transition-colors"
                      whileHover={{ scale: 1.02, x: 5 }}>
                      <span className="text-xs uppercase tracking-wider text-muted-foreground">{exp.type}</span>
                      <h4 className="font-display font-bold text-base sm:text-lg mt-1">{exp.title}</h4>
              ...
              <StaggeredContainer className="space-y-6 sm:space-y-8 md:space-y-10" staggerDelay={0.1} direction="right">
                {virtualInternships.map((exp) => (
                  <div key={exp.title + exp.company} className="relative pl-8 sm:pl-10">
                    ...
                    <motion.div className="glass-card card-glow rounded-lg p-4 sm:p-5 md:p-6 hover:bg-card/80 transition-colors"
                      whileHover={{ scale: 1.02, x: 5 }}>
                      <span className="text-xs uppercase tracking-wider text-muted-foreground">Virtual Internship</span>
                      <h4 className="font-display font-bold text-base sm:text-lg mt-1">{exp.title}</h4>
                      <p className="text-sm sm:text-base text-muted-foreground mt-1">{exp.company}</p>
                    </motion.div>
                  </div>
