  ...
  return (
    <div className="relative overflow-hidden py-3">
      <motion.div className="flex gap-4"
        animate={{ x: direction === 'left' ? [0, -33.33 * skills.length * 3] : [-33.33 * skills.length * 3, 0] }}
        transition={{ duration: 40, repeat: Infinity, ease: 'linear' }}>
        {duplicatedSkills.map((skill, index) => (
          <motion.span key={`${skill}-${index}`}
    ...
    <section id="skills" className="py-16 sm:py-24 md:py-32 relative bg-card/30" ref={ref}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 md:px-12">
        <motion.div className="text-center mb-10 sm:mb-16 md:mb-20"
          ...
          <span className="text-xs sm:text-sm uppercase tracking-[0.2em] sm:tracking-[0.3em] text-muted-foreground font-display">Expertise</span>
        ...
        <StaggeredContainer className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 md:gap-8" staggerDelay={0.12} direction="up">
          {skillCategories.map((category) => (
            <motion.div key={category.title} className="glass-card card-glow rounded-lg p-4 sm:p-5 md:p-6" whileHover={{ scale: 1.02 }}>
              <h3 className="font-display font-bold text-lg sm:text-xl mb-4 sm:mb-6 text-foreground">{category.title}</h3>
                ...
                {category.skills.map((skill, skillIndex) => (
                  <motion.div key={skill.name}
                    ...
                    <div className="h-2 bg-secondary/50 rounded-full overflow-hidden">
                      <motion.div className="h-full bg-gradient-to-r from-silver via-platinum to-titanium rounded-full"
                        initial={{ width: 0 }} whileInView={{ width: `${skill.proficiency}%` }} viewport={{ once: true }}
                        transition={{ duration: 1, delay: skillIndex * 0.1, ease: 'easeOut' }}
                        style={{ boxShadow: '0 0 10px hsl(var(--glow-silver))' }} />
                    </div>
                  </motion.div>
                ))}
              </div>
        ...
        <motion.div className="mt-12 sm:mt-16 md:mt-20 -mx-4 sm:-mx-6 md:-mx-12"
