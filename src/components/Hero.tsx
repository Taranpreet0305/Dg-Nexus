  ...
  return (
    <section ref={ref} className="relative min-h-[100dvh] flex items-center justify-center overflow-hidden px-4">
      ...
      <motion.div
        ...
        <motion.h1 className="text-4xl sm:text-5xl md:text-7xl lg:text-8xl xl:text-9xl font-display font-bold tracking-tight mb-4 sm:mb-6"
        initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.2 }}>
          <span className="block glow-text text-foreground">DAKSH</span>
          <span className="block text-gradient">GULATI</span>
        </motion.h1>

        <motion.div className="h-6 sm:h-8 md:h-10 overflow-hidden mb-8 sm:mb-10 md:mb-12"
        initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.5 }}>
          <motion.div
            ...
            <div key={index} className="h-6 sm:h-8 md:h-10 flex items-center justify-center text-xs sm:text-sm md:text-base lg:text-lg font-body tracking-[0.15em] sm:tracking-[0.2em] md:tracking-[0.3em] text-muted-foreground uppercase">
                {title}
              </div>
            )}
          </motion.div>
        ...
        <motion.div className="flex flex-col sm:flex-row sm:gap-4 justify-center items-center gap-[35px]"
        initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.7 }}>
          <MagneticButton as="a" href="#work" className="btn-outline w-full sm:w-auto text-sm sm:text-base"
          strength={0.3} radius={100}
          onClick={(e) => {e.preventDefault();document.querySelector('#work')?.scrollIntoView({ behavior: 'smooth' });}}>
            View Work
          </MagneticButton>
          <MagneticButton as="a" href="#contact" className="btn-outline w-full sm:w-auto text-sm sm:text-base"
          strength={0.3} radius={100}
      ...
      <motion.div className="absolute bottom-6 sm:bottom-10 left-1/2 -translate-x-1/2"
